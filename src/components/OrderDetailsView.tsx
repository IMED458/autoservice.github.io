...
import { Wrench, User as UserIcon, Phone, Calendar, Plus, Trash2, Check, Edit, Save, ArrowLeft, Settings } from 'lucide-react';
...
export default function OrderDetailsView({
  ...
  const handleSaveEditServiceInline = () => {
    ...
    setDraftServices(prev => prev.map(s => s.id === editingSrvId
      ? { ...s, serviceType: editSrvType, description: editSrvDesc.trim(), price: Number(editSrvPriceValue), mechanicId: editSrvMechId, mechanicEarning: earn }
      : s
    ));
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 mb-5 shadow-xl relative overflow-hidden">
        ...
        {!isEditingAdmin ? (
          <div>
            ...
            {/* Problem description - always editable for all */}
            <div className="mb-4">
            ...
            <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <button type="button" onClick={() => setShowAddServiceForm(false)}
                  className="py-1.5 bg-slate-900 border border-slate-800 text-slate-400 text-xs font-semibold rounded-lg cursor-pointer">
                  გაუქმება
                </button>
                <button type="submit"
                    ...
                    <div className="space-y-2 text-xs">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <select value={editSrvType} onChange={e => setEditSrvType(e.target.value)}
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex-1 space-y-1">
                        <div className="flex flex-wrap items-center gap-x-2">
                          ...
                          <span className="text-[10px] text-slate-500">ხელოსანი: <b className="text-slate-400">{mechObj.firstName} {(mechObj as any).lastName}</b></span>
                        </div>
                        ...
                        <div className="text-[10.5px] text-slate-400 flex items-center gap-2">
                          ...
                          <span>გამომუშავება: <b className="text-cyan-400 font-mono">{srv.mechanicEarning} ₾</b></span>
                        </div>
                      </div>
