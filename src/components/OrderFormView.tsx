...
import { CarServiceOrder, OrderStatus, PaymentStatus, CarBrand } from '../types';
import { Calendar, Car, User, Phone, Plus, FileText, CheckCircle, ChevronDown } from 'lucide-react';
...
export default function OrderFormView({ carBrands, onAddOrder, onCancel }: OrderFormViewProps) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className="max-w-lg mx-auto p-4 pb-28 bg-slate-950 text-slate-100 font-sans"
    >
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={onCancel} className="text-xs text-slate-400 hover:text-slate-200 bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-lg">
          უკან
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl">
        ...
        <div>
          ...
          <div ref={brandRef} className="relative">
            <div
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-sm text-slate-200 focus-within:border-amber-500/60 flex items-center justify-between cursor-pointer"
            >
              <input
                type="text"
                ...
                className="bg-transparent outline-none flex-1 placeholder-slate-600 text-slate-200"
              />
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-20 max-h-44 overflow-y-auto">
                {filteredBrands.length === 0 ? (
                  ...
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-sm font-mono font-bold text-slate-200 hover:bg-amber-500/10 hover:text-amber-400 transition-colors"
                  >
                    {b.name}
                  </button>
                ))}
        ...
        <div>
          ...
          <input
            ...
            lang="en"
            autoCapitalize="characters"
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-amber-500/60 placeholder-slate-600 uppercase"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            autoComplete="off"
            maxLength={9}
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-sm uppercase text-slate-200 font-mono tracking-widest focus:outline-none focus:border-amber-500/60 placeholder-slate-600"
          />
        </div>
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-400 mb-1 flex items-center gap-1.5">
            <User className="w-3.5 h-3.5 text-slate-500" /> კლიენტის სახელი და გვარი
          </label>
          <input
            type="text"
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-amber-500/60 placeholder-slate-600 font-mono"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <div>
            ...
            <select value={status} onChange={e => setStatus(e.target.value as OrderStatus)}
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-amber-500/60">
        ...
        <div className="grid grid-cols-2 gap-3 pt-3">
          <button type="button" onClick={onCancel}
            className="w-full py-2.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-300 font-bold rounded-xl text-xs cursor-pointer">
            გაუქმება
          </button>
          <button type="submit"
            className="w-full py-3 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-bold rounded-xl text-xs cursor-pointer flex items-center justify-center gap-1.5">
