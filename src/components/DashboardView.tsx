...
export default function DashboardView({
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    // For plate comparison, also try the formatted version so "AB123" matches "AB-123-CD"
              ...
              >
                <div
                  className={`absolute left-0 top-0 bottom-0 w-1.5 ${
                    order.status === 'new' ? 'bg-amber-500' : order.status === 'pending' ? 'bg-sky-500' : 'bg-green-500'
                  }`}
                />

                <div className="flex items-start justify-between mb-2.5 pl-2">
                  <span className="text-sm font-bold text-slate-200 uppercase font-sans tracking-tight">{order.carBrand}</span>
                ...
                <div className="flex items-center justify-between border-t border-slate-800/80 pt-3 pl-2">
                  <div className="flex gap-1.5">
                    <span className={`text-[10px] uppercase tracking-wider font-extrabold px-2.5 py-1 rounded-lg border ${statusStyle.bg} ${statusStyle.border} ${statusStyle.text} font-sans`}>
                      {ORDER_STATUS_LABELS[order.status]}
                    </span>
                    <span className={`text-[10px] uppercase tracking-wider font-extrabold px-2.5 py-1 rounded-lg border ${payStyle.bg}`}>
